...
type HttpStrategyHandler struct {
	strategyUsecase usecase.StrategyUsecases
}

func NewHttpStrategyHandler(strategyUseCase usecase.StrategyUsecases) *HttpStrategyHandler {
...
func (h *HttpStrategyHandler) GetEventStrategyByID(c *fiber.Ctx) error {
	...
	eventStrategy, err := h.strategyUsecase.GetEventStrategyByID(uint(eventStrategyID))
	if err != nil {
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
			"error": err.Error(),
		})
...
func (h *HttpStrategyHandler) GetAllEventStrategies(c *fiber.Ctx) error {
	eventStrategies, err := h.strategyUsecase.GetAllEventStrategy()
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"message": err.Error(),
		})
